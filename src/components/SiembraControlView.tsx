      ...
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-8">
        <div className="flex justify-between items-center bg-white/5 p-8 rounded-[3rem] border border-white/10 backdrop-blur-md">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-white/20 font-bold uppercase text-[10px] tracking-widest mt-1">Motor de Gestión de Proyecto</p>
            </div>
          </div>
          <div className="px-8 py-4 bg-[#10B981] text-black rounded-2xl font-black tracking-widest shadow-2xl shadow-emerald-500/20 uppercase text-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 flex flex-col gap-8">
            <div className="bg-white/5 rounded-[3rem] border border-white/10 p-8 flex flex-col gap-6 backdrop-blur-md">
              <div className="flex flex-col gap-6 border-b border-white/5 pb-6">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-white/20 font-bold uppercase text-[10px] tracking-widest mt-1">Registro cronológico completo</p>
                  </div>
                  <button 
                    onClick={() => setIsActivityModalOpen(true)}
                    className="flex items-center gap-2 px-8 py-4 bg-[#3B82F6] text-white rounded-2xl font-black shadow-2xl shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all uppercase text-xs tracking-widest"
                  >
                ...
                <div className="flex flex-col md:flex-row items-center gap-4">
                  <div className="relative flex-1 w-full">
                    ...
                    <input 
                      ...
                      className="w-full h-14 pl-12 pr-4 bg-white/5 rounded-2xl border border-white/10 focus:border-[#3B82F6] font-bold outline-none transition-all text-sm text-white"
                    />
                  </div>
                  <div className="flex items-center gap-3 bg-white/5 p-1 rounded-2xl border border-white/10">
                    <button 
                      ...
                      className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center disabled:opacity-10 transition-all active:scale-90 hover:bg-white/10"
                    >
                      <ChevronLeft size={20} />
                    </button>
                    <span className="text-[10px] font-black uppercase tracking-widest px-2 text-white/40">
              ...
              <div className="flex flex-col gap-4 min-h-[400px]">
                {paginatedActivities.length === 0 ? (
                  <div className="p-12 text-center flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-white/20 font-bold uppercase text-xs tracking-widest">No se encontraron actividades</p>
                  </div>
                ) : (
                  paginatedActivities.map(log => (
                    <div key={log.id} className="p-6 bg-white/5 rounded-[2rem] border border-white/5 flex flex-col gap-4 hover:bg-white/[0.07] transition-colors">
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <span className="text-lg font-black block tracking-tight text-white">{log.description}</span>
                            <div className="flex flex-wrap items-center gap-3 mt-1">
                              ...
                              {log.consumedReceptionId && (
                                <span className="text-[10px] font-black text-[#3B82F6] uppercase tracking-widest bg-blue-500/10 px-2 py-0.5 rounded">
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            ...
            <div className="bg-white/5 rounded-[3rem] border border-white/10 p-8 flex flex-col gap-6 backdrop-blur-md">
              <div className="flex items-center gap-4 border-b border-white/5 pb-6">
                ...
                <div>
                  ...
                  <p className="text-white/20 font-bold uppercase text-[10px] tracking-widest">Control de densidad</p>
                </div>
              </div>

              <div className="flex flex-col gap-6">
                <div className="flex flex-col gap-2">
                  <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">Fecha de Siembra</span>
                  <input 
                    type="date" 
                    value={newSowing.date}
                    onChange={e => setNewSowing(prev => ({ ...prev, date: e.target.value }))}
                    className="h-14 px-6 bg-white/5 rounded-2xl font-black text-lg outline-none focus:ring-2 focus:ring-[#10B981] transition-all text-white border border-white/10"
                  />
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">Responsable</span>
                  <select 
                    value={newSowing.responsibleId}
                    onChange={e => setNewSowing(prev => ({ ...prev, responsibleId: e.target.value }))}
                    className="h-14 px-6 bg-white/5 rounded-2xl font-black appearance-none outline-none focus:ring-2 focus:ring-[#10B981] transition-all text-white border border-white/10"
                  >
                ...
                <button 
                  ...
                  className="h-16 bg-[#10B981] text-black font-black rounded-2xl shadow-2xl shadow-emerald-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all uppercase tracking-widest text-sm"
                >
                  Registrar Siembra
                </button>
              </div>
            ...
            {/* Dispatch Section */}
            <div className="bg-[#3B82F6] rounded-[3rem] shadow-2xl shadow-blue-500/20 p-8 flex flex-col gap-6 text-white">
                  ...
                  {projectLots.map(lot => {
                    ...
                    return (
                      <div key={lot.id} className="flex justify-between items-center">
                        ...
                        <span className={`text-[10px] font-black px-2 py-1 rounded ${isApproved ? 'bg-[#10B981] text-black' : 'bg-red-500/20 text-red-200'}`}>
                ...
                <button 
                  ...
                  className={`h-16 font-black rounded-2xl shadow-lg transition-all uppercase tracking-widest text-sm ${
                    projectLots.some(l => l.analyses?.some(a => a.status === 'APROBADO')) 
                      ? 'bg-white text-[#3B82F6] hover:scale-[1.02] active:scale-[0.98]' 
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col gap-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white/5 p-8 rounded-[3rem] border border-white/10 backdrop-blur-md">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-emerald-500/10 text-[#10B981] rounded-2xl flex items-center justify-center shadow-2xl shadow-emerald-500/20 border border-emerald-500/20">
            <Sprout size={32} />
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="#3B82F6 font-black text-xs">{currentPage} de {totalPages}</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full h-14 pl-12 pr-4 bg-white/5 rounded-2xl border border-white/10 focus:border-[#3B82F6] font-bold outline-none transition-all text-white"
            />
          </div>
          <div className="flex gap-2">
            <button 
              ...
              className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center disabled:opacity-10 hover:bg-white/10 transition-colors shadow-sm"
            >
              <ChevronLeft size={24} />
            </button>
            <button 
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 h-14 px-8 bg-[#3B82F6] text-white rounded-2xl font-black shadow-2xl shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all whitespace-nowrap uppercase text-xs tracking-widest"
          >
      ...
      <div className="bg-white/5 rounded-[3rem] border border-white/10 p-8 flex flex-col gap-6 backdrop-blur-md">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedProjects.length === 0 ? (
            <div className="col-span-full text-center py-20 opacity-20 font-black uppercase tracking-widest">No se encontraron proyectos</div>
          ) : (
            paginatedProjects.map(project => (
              <button
                key={project.id}
                onClick={() => setSelectedProjectId(project.id)}
                className="p-8 rounded-[2.5rem] text-left transition-all flex flex-col gap-6 bg-[#10B981] text-black border border-transparent hover:-translate-y-2 active:scale-95 shadow-2xl shadow-emerald-500/10 group relative overflow-hidden h-[280px]"
              >
                <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform">
                  <Sprout size={120} />
                </div>
                <div className="flex justify-between items-start w-full z-10">
                  <div className="p-4 rounded-2xl bg-black/10 text-black flex items-center justify-center">
                    <Sprout size={24} />
                  </div>
                  <ChevronRight size={24} className="text-black/30 group-hover:text-black transition-colors" />
                </div>
                <div className="z-10 mt-auto">
                  <div className="font-black text-3xl text-black tracking-tight leading-none">{project.name}</div>
                  <div className="text-[10px] font-black uppercase tracking-widest mt-3 text-black/60 flex items-center gap-2">
                    <Calendar size={14} /> Inicio: {new Date(project.startDate).toLocaleDateString()}
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-black/10 flex justify-between items-center z-10">
                  <div className="flex items-center gap-2 text-black/60">
                    <List size={16} />
                    <span className="text-xs font-black">{project.activityLogs.length} Actividades</span>
                  </div>
                  <div className="text-[10px] font-black tracking-widest px-3 py-1 rounded-lg bg-black/10 text-black uppercase">
                    {project.status}
                  </div>
                </div>
              </button>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-zinc-900 w-full max-w-xl rounded-[3rem] p-10 shadow-2xl flex flex-col gap-8 border border-white/10">
              <div>
                <h3 className="text-3xl font-black tracking-tight uppercase text-white">Nuevo Proyecto</h3>
                <p className="text-white/20 font-bold uppercase text-[10px] tracking-widest mt-1">Configuración inicial de siembra</p>
              </div>

              <div className="flex flex-col gap-6">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black text-white/20 uppercase tracking-widest">Nombre del Proyecto</label>
                  <input type="text" value={newProject.name} onChange={e => setNewProject({ ...newProject, name: e.target.value })} className="h-14 px-6 bg-white/5 rounded-2xl font-bold outline-none focus:ring-2 focus:ring-[#3B82F6] text-white border border-white/10" placeholder="Ej: Zafra 2024 - Lote 01" />
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black text-white/20 uppercase tracking-widest">Lotes Asociados</label>
                    ...
                    {lots.map(lot => (
                      <button
                        ...
                        className={`p-4 rounded-xl font-bold text-sm transition-all border-2 ${
                          newProject.lotIds.includes(lot.id) ? 'bg-emerald-500/10 border-emerald-500 text-emerald-500' : 'bg-white/5 border-transparent text-white/20'
                        }`}
                      >
                        {lot.lotCode}
                      </button>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black text-white/20 uppercase tracking-widest">Fecha de Inicio</label>
                  <input type="date" value={newProject.startDate} onChange={e => setNewProject({ ...newProject, startDate: e.target.value })} className="h-14 px-6 bg-white/5 rounded-2xl font-bold outline-none text-white border border-white/10" />
                </div>

                <div className="flex gap-4 pt-4">
                  <button onClick={() => setIsModalOpen(false)} className="flex-1 h-14 bg-white/5 text-white/40 rounded-2xl font-black uppercase tracking-widest text-xs border border-white/10">Cancelar</button>
                  <button onClick={() => {
                  ...
                  }} className="flex-1 h-14 bg-[#3B82F6] text-white rounded-2xl font-black shadow-2xl shadow-blue-500/20 uppercase tracking-widest text-xs">Crear Proyecto</button>
                </div>
              </div>
            </motion.div>
          </div>
